...
export default class FontSizeEditing extends Plugin {
  ...
  _prepareAnyValueConverters(definition) {
    ...
    const presets = definition.model.values.filter(
      value => !String(value).match(/[\d.]+[\w%]+/),
    );
    ...
    editor.conversion.for('downcast').attributeToElement({
      ...
      view: (attributeValue, writer) => {
      	console.log('downcast -> ', FONT_SIZE, attributeValue, writer);
        if (!attributeValue) {
          return;
        }
    ...
    editor.conversion.for('upcast').attributeToAttribute({
      model: {
        key: FONT_SIZE,
        value: viewElement => {
        	const value = viewElement.getStyle('font-size');
					console.log('upcast -> ', FONT_SIZE, value, viewElement);
